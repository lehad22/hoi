...
def get_db() -> psycopg.Connection:
    if 'db' not in g:
        g.db = psycopg.connect('postgresql://postgres:admin@127.0.0.1/demo')
...
class Cities(Resource):
    def get(self):
        ...
        cur.close()
        return {
            'departure': list(map(lambda a: a[0], departure_cities)),
            'arrival': list(map(lambda a: a[0], arrival_cities)),
        }
...
class Airports(Resource):
    def get(self):
        ...
        cur.close()
        return {
            'departure': list(map(lambda a: a[0], departure_airports)),
            'arrival': list(map(lambda a: a[0], arrival_airports)),
        }


class AirportsByCity(Resource):
    def get(self, city):
        cur = get_db().cursor()
...
""",
            [airport])
        r = cur.fetchall()
        result = list(map(lambda a: {'airport': a[0],
                                     'origin': a[1],
                                     'flight_no': a[2],
                                     'time': a[3][0].strftime("%H:%M:%S"),
                                     'day_of_week': a[4]}, r))

        if len(result) == 0:
            return make_response(jsonify({'error': 'Airport not found'}), 404)
        return result
...
class Booking(Resource):
    def post(self):
        json = request.json
        print(json)
        print(json['passenger_name'])
        print(json['flight_ids'])
        print(json['fare_conditions'])
...
api.add_resource(AirportsByCity, '/cities/<city>/airports')
api.add_resource(InboundSchedule, '/airports/<airport>/inbound_schedule')
api.add_resource(OutboundSchedule, '/airports/<airport>/outbound_schedule')
